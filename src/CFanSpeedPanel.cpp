    ...
    , mMousePositionY(0)
{
	...
	if (wxFileExists(mFanProfileDataFilePath))
	{
		...
		if (doc.GetRoot()->GetName() == wxT("FAN_PROFILE"))
		{
			...
			{
			    wxString nr_of_samples = child->GetAttribute(wxT("samples"), wxT("0"));
			    nr_of_samples.ToLong(&mFanProfileDataValid);
			}
			else if (child->GetName() == wxT("SAMPLE"))
			{
			    wxString percentage = child->GetAttribute(wxT("percentage"), wxT("0"));
			    wxString fanspeed = child->GetAttribute(wxT("fanspeed"), wxT("0"));
...
{
    wxPaintDC dc(mFanProfile);

    dc.SetBrush(wxBrush(mFanProfile->GetBackgroundColour()));
